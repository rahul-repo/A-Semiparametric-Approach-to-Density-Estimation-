...
This project is a group academic study on semiparametric density estimation, combining parametric modeling with nonparametric kernel density estimation. The objective is to construct a flexible density estimator that balances structural assumptions with data-driven adaptability.
...
- $f(x, \hat{\theta})$ is a parametric density estimated via maximum likelihood estimation  
- $\hat{f}(x)$ is a kernel density estimator (KDE)  
- $\pi \in [0,1]$ is a mixing parameter estimated numerically  
...
- Behavior under parametric vs nonparametric dominance  
- Convergence properties under different assumptions  
- Sensitivity analysis with respect to $\pi$  
- Comparison with standalone parametric and KDE models  
